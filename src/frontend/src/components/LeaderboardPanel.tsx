  ...
  return (
    <div className="space-y-3 mt-6">
      {leaderboard.slice(0, 10).map((profile, index) => {
        const averageRating =
          profile.ratingCount > 0n
            ? (Number(profile.ratingSum) / Number(profile.ratingCount)).toFixed(
                1,
              )
            : "N/A";
        ...
        return (
          <div
            // biome-ignore lint/suspicious/noArrayIndexKey: leaderboard rank is the stable key
          ...
          >
            <div
              className={`w-10 h-10 rounded-full flex items-center justify-center font-bold flex-shrink-0 ${
                index === 0
                  ? "bg-[oklch(0.8_0.25_150)]/20 text-[oklch(0.8_0.25_150)]"
                  : index === 1
                    ? "bg-[oklch(0.7_0.2_270)]/20 text-[oklch(0.7_0.2_270)]"
                    : index === 2
