...
export function useGoogleAuth(): UseGoogleAuthReturn {
  ...
  useEffect(() => {
    ...
    if (!isLoaded) {
      ...
      timeoutId = setTimeout(() => {
        if (!isLoaded) {
          console.error('Google Identity Services SDK failed to load within timeout');
  ...
  const handleCredentialResponse = useCallback((response: GoogleAuthResponse) => {
  ...
  const signIn = useCallback(async (): Promise<string | null> => {
    if (!isLoaded || !isInitialized) {
      const errorMsg = 'Google Sign-In not ready. Please wait or refresh the page.';
